...
*
dtls_set_record_header(uint8_t
type,
...
dtls_int_to_uint16(buf,
DTLS_VERSION);
buf += sizeof(uint16_t);

if (security) {
dtls_int_to_uint16(buf, security
->epoch);
buf += sizeof(uint16_t);

dtls_int_to_uint48(buf, security
->rseq);
buf += 6; /* 48 bits */
...
memset(buf,
0, sizeof(uint16_t) + 6); /* 16 + 48 bits */
...
static inline uint8_t
*
dtls_set_handshake_header(uint8_t
type,
dtls_peer_t *peer,
int length,
int frag_offset,
...
dtls_int_to_uint16(buf, peer
->handshake_params->hs_state.mseq_s);
...
finalize_hs_hash(dtls_peer_t
*peer,
uint8_t *buf
) {
return
dtls_hash_finalize(buf,
...
dtls_send_server_hello_msgs(dtls_context_t *ctx, dtls_peer_t *peer) {
    ...
    if (is_tls_psk_with_aes_128_ccm_8(peer->handshake_params->cipher)) {
        ...
        if (len > 0) {
            res = dtls_send_server_key_exchange_psk(ctx, peer, psk_hint, (size_t)
            len);
...
decrypt_verify(dtls_peer_t *peer, uint8_t *packet, size_t length,
    ...
    *cleartext = (uint8_t * )
    packet + sizeof(dtls_record_header_t);
